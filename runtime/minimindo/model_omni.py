# -*- coding: utf-8 -*-
# MiniMind-O 官方模型代码（纯文本裁剪版，Apache-2.0）
# 来源：github.com/jingyaogong/minimind-o  model/model_omni.py（master）
# 裁剪说明：仅保留文本推理所需代码。移除顶层 soundfile/librosa/onnxruntime/numpy
# 依赖（仅语音/视觉路径使用），移除 SileroVAD / RealtimeSession（VAD 由 runtime 提供）。
# MiniMindOmni.__init__ 的 audio_encoder_path / vision_model_path 传空路径时
# 自动返回 (None, None)，文本推理无需 SenseVoice / SigLIP2 / Mimi。
import os, math, torch, warnings, logging, contextlib, io
