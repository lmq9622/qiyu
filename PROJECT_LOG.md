...
- 后端：FastAPI + 意图路由 + 记忆系统 + RAG + 微信接入，LLM 指向 x99 内网 llama.cpp（`192.168.2.6:8081`），OpenAI 兼容格式，可随时换 API。
...
**本次更新内容**
**① x99 双卡并行模式压测（192.168.2.6 · Qwen3.6-35B-A3B-uncensored-heretic Q4_K_M · llama-server -np 4）**
...
- 未跑完原因：用户机器上 AI Novel Writing Assistant v2 用 6 条长连接占用同一 llama.cpp（192.168.2.6:8081），栖语每次生成请求都要排队数分钟，测试台 240s 超时 → topic_shift 段开始级联超时；另发现 D:/Codex projects/ai量化（QuantMachine）占用 127.0.0.1:8766，开发后端已改到 8768 避开。
...
### M6 Realtime Brain 多后端 / 多组件 / 外围全套（本次，规格§3-§9/§11-§13/§15/§17/§25/§36/§40-§49）
- **Realtime Brain 真后端（规格§4/§6/§11）**：`runtime/realtime.py` 重写——`CPUBackend / VulkanBackend / CUDABackend`（llama.cpp GGUF 优先，onnxruntime 备选，不写死 Vulkan/CUDA），9 组件（Thinker/Talker/SenseVoice/SigLIP2/Mimi/CAMPPlus/VAD/AudioCodec/VisionEncoder）各自独立 Device/Backend 计划（`/v1/runtime/models` 返回组件计划）；`models/realtime/` 无权重或运行时缺失时用 `UnavailableRealtimeBackend` 诚实回退 Main Brain（规格§54）。
- **MiniMind-O 真实边界**：官方 Omni 权重当前以 PyTorch 发布、无官方 GGUF；能真装的部分（文本 Thinker via llama.cpp）已提供 `tools/install_models.py`（Hardware Detection → Recommended Model/Backend → 下载到 `models/realtime/thinker.gguf`），全模态（听/说/看）按多组件架构预留并如实说明，绝不假装已部署。
- **MicroBenchmark（规格§13）**：新 `runtime/benchmark.py`——候选 backend 有真实 `bench_inference` 就实测（`measured=True`），否则硬件启发式并如实标注 `measured=False`；RuntimeManager 启动后异步跑一次。
- **统一并发闸（规格§25）**：新 `runtime/concurrency.py`——`GlobalConcurrencyLimiter`，parallel_requests=auto/1/2/3/4/unlimited，auto 按 CPU/RAM 决定，分闸（llm/tool/memory/vision/tts/stt/avatar）工具联网类更保守；`companion/active.py` 的 `_LLMLimiter` 与后台 tool 并发全部委托到总闸。
- **多消息流水线 / 会话投入 / 动态冷却（规格§15/§17/§36）**：新 `companion/pipeline.py`——`PendingMessageQueue` + `build_pending_messages_block`（连续多条用户消息作为结构化批次注入系统提示词）；`conversation_engagement_state`（讲故事后听众不在场→等待→收尾）；`conversation_cooldown_seconds`（关系/耐心/聊天长度/活跃动态决定）。demo 流式与非流式路径均传 `pending_messages`。
- **视觉 / 语音 / 嵌入 / Avatar / 平台（规格§40-§44/§46/§8）**：新 `runtime/vision.py`（主大脑多模态直通 `MainBrainVisionProvider`）、`runtime/stt.py`（sherpa-onnx 本地识别）、`runtime/tts.py`（Windows SAPI 真实合成 + edge-tts 可选，支持 speed/pitch/voice/emotion 参数）、`runtime/embedding_provider.py`（bge-small-zh-v1.5，探测用 find_spec 不阻塞事件循环，首次调用懒加载）、`runtime/avatar.py`（`AvatarProvider / Live2DAvatarProvider / VRCAvatarProvider / GenericAvatarController` + 视频通话 CameraInput/AudioInput/AudioOutput/AvatarRenderer 预留）、`runtime/platform.py`（`MessagePlatformProvider`：WeChat 现在 / Telegram·Discord 未来，Incoming/OutgoingMessage 统一）。
- **统一 DB / 分层日志 / 性能监控（规格§47/§48/§49）**：新 `runtime/db.py`（sqlite3 统一 8 表：conversations/messages/memories/relationship_state/emotion_state/tool_calls/proactive_events/tasks，与现有 JSON 热路径并行落库）；新 `runtime/logging_setup.py`（runtime/conversation/tool/memory/performance/error 六类独立文件）；新 `runtime/perf.py`（PerformanceMonitor：TTFT/decode tok/s/记忆检索/工具/STT/TTS/avatar/视觉延迟 + 资源占用，`/v1/perf`）。聊天/工具/记忆/调度/主动消息均已埋点写日志与统一表。
- **打包（规格§10）**：`build.py` VERSION 0.0.18→0.0.19，hidden_imports 补 `runtime.logging_setup/perf/concurrency/benchmark/vision/stt/tts/embedding_provider/avatar/platform/db/main_brain`、`companion.pipeline`、`tools/tools.web/tools.install_models`；sidecar `dist/tools/` 复制安装器与联网工具箱、`dist/models/README.txt` 说明安装器用法。
- **修复**：`companion/active.py` 缺 `import httpx`（搜索子代理规划因此从未真正生效，一直走兜底关键词）；`runtime/avatar.py` 头像延迟写死 0.5ms→真实计时；`runtime/perf.py` 补 `vision_latency` 指标；embedding 探测 25s 阻塞事件循环→find_spec 毫秒级。`tools/` 被 .gitignore 覆盖导致 `tools/web.py`、`tools/install_models.py` 未纳入版本——已 `git add -f` 强制跟踪（fresh clone 可用 ToolAgent 与安装器）。
- **验证**：`python -m compileall` 全绿；隔离启动（`QIYU_PORT=8768 QIYU_DATA_DIR=test_harness\.tmpdata`）端点冒烟全绿（providers/models/avatar/perf/platforms/tts/tts-status/stt/stt-status/realtime-judge/models-load-unload）；真实 TTS 合成 WAV 200；统一 DB 消息/tool_calls/关系/情绪/任务落库；行为回归 s501-s526 26/26 通过、工具类 s081-s090 10/10 通过（真实执行 8/10）；日志六类文件均产出内容。

### M7 六个 unavailable Provider 全部真实落地（本次，规格§8/§40-§44/§46/§53）
- **STT 真实可用**：sherpa-onnx 1.13.7 + paraformer-zh 小模型安装到 models/asr/（自动发现，不再只认 data/asr）；channels/wechat_voice.py 支持单文件/双文件 ASR 模型；runtime/tts.py SAPI 输出改为 16kHz mono 并修复 duration_ms 单位 bug——实测「TTS 合成→16kHz→STT 转写」端到端识别成功。/v1/stt/status available=True。
- **Vision 真实探测**：runtime/vision.py 的视觉支持判定从「读静态设置」改为「启动时真实探测」——发一张 1x1 测试图给主模型，能返回非空内容才 available（模型/地址变化自动重探测）。本机实测主模型（qwen3.6-35b 多模态）探测通过，main-brain-vision available=True。
- **Realtime Brain 真实部署**：装好 llama-cpp-python 0.3.35（GitHub 预编译 py3-none win_amd64 轮子，PyPI 无 cp313 轮子），下载 MiniMind2-gguf Q4-MiniMind2.gguf 到 models/realtime/thinker.gguf（60.7MB，0.3B）；runtime/hardware.py 的 Vulkan 能力从写死 unavailable 改为真实探测 llama.cpp Vulkan 后端，本机 AMD RX 6700 XT 实测 minimindo-vulkan available=True、/v1/runtime/realtime/judge 走真实 GPU 推理（约 200ms）。诚实说明：0.1-0.3B 量化模型对「JSON 决策输出」遵循不稳定，judge 输出不可解析时如实回退 Main Brain 并在 reason 里附上模型原文（规格§54，不假装能实时处理）。
- **VRC Avatar 真实 OSC 桥**：runtime/avatar.py 的 VRCAvatarProvider 从占位改为真实 VRChat OSC 发送端（stdlib socket+struct，UDP 9000）：render() 把 emotion/intensity/action/expression/talking/speed 映射为 /avatar/parameters/… 并真实发送；probe() 检测 VRChat 进程（psutil）或显式 QIYU_VRC_OSC_PORT 端口可达才 available，未运行如实上报。
