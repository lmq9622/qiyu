...
### 2026-09-10 晚 · 用户真机反馈后的修复

用户反馈三件事，逐条处理：

| 反馈 | 根因 | 修复 |
|---|---|---|
| “我人都能走，为啥角色走不了” | agent 半径 0.25 m 的腐蚀太狠：仅 12.5 m 墙周长就吃掉约 3 m²，小房间直接归零 | 导航半径降到 0.16（角色碰撞体同步 0.25→0.18）；新增**用户足迹覆盖率**日志，用“用户真实站过的位置是否在 NavMesh 上”作为判据，而不是靠几何推导 |
| “人没有手肘，直接连到身体” | 手臂画成 肩→中点→手 的直线；且手没跟到时会回退成头的位置，看起来就像长在身体上 | 加入**双骨 IK 求肘**（肩→肘→手，肘朝下略外扩略后，即 VRChat/SteamVR 三点动捕做法）；手不可信时整条手臂隐藏；手超出臂长会夹回可达范围 |
| “整个骨架是飘的” | 髋部由头部高度推算、没有腿，整个人悬在空中 | 髋部改为**落在地面高度**（0.53×身高），补上髋→膝→踝的腿与脚，脚踩真实地面；手部位置加指数平滑 |

角色模型：用户提供的 MMD 模型已跑通 `.pmx → FBX → Unity Humanoid`，
Unity 导入日志确认 `Humanoid=True`。流程与踩坑见
[AVATAR_PMX_TO_UNITY.md](AVATAR_PMX_TO_UNITY.md)。
模型为「禁止二次配布」，**不入库**，只在本地转换与验收。
